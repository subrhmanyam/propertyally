...
- `admin_create_service_request` (`POST /admin`) and `update_service_request` (`PATCH /{id}`) previously had **zero authorization** — any caller reaching the API (including a tenant hitting it directly, bypassing `/tenant-decision`) could change any request's status or create one for any unit. Both now require a `user_id` query param and call `require_org_admin_for_unit(sb, user_id, leasing_unit_id)` before mutating; `tenant_decide_service_request` was already correctly self-scoped (unchanged).
- This is the reference implementation the org-admin pattern was later extracted from into `backend/auth_utils.py` (`require_org_admin`, `require_org_admin_for_unit`, `require_any_org_admin`) — `service_requests.py` was refactored to import from there instead of keeping its own local copy.
- `service_catalog.py`'s admin mutations (`POST/PUT/DELETE /`) now require `user_id` + `require_any_org_admin` (the catalog is global, not org-scoped, so any org admin — not just one specific org's — may manage it). No live frontend caller for these three exists yet, so nothing on the Dart side needed updating.
- Frontend: `services_repository.dart` now has a `_userId` getter (`Supabase.instance.client.auth.currentUser?.id`) threaded as a `user_id` query param into `createAdminRequest` and `updateRequest`.
- Deliberately left alone: `GET /` and `GET /{request_id}` (list/read) are still unauthenticated — a tenant session that transiently reaches the admin screen could still *view* all requests, just not *change* anything. Flagged, not fixed, per explicit scope decision that session.
