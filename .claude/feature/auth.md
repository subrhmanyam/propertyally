...
- Key endpoints:
  ...
  - `GET /api/v1/auth/profile?user_id=` — returns `{id, role, full_name, email}` for a user; used by the frontend as a fallback role lookup when the JWT doesn't yet carry `app_metadata.role`. **Checks the `tenants` table first** (matched via `auth_user_id`) and returns `role: 'tenant'` if found — only falls through to the `profiles` table (404 if no row there either) when no tenants row matches. See Recent changes below for why this order matters.
...
## Recent changes
- **Root-cause fix for tenants being silently treated as admin**: `AuthRepository.fetchRole()` (frontend) catches *any* error from `GET /api/v1/auth/profile` — including a 404 — and falls back to `'admin'`. Tenant accounts that never went through `POST /register` (e.g. created directly against a `tenants` row, or via a raw SQL insert into `auth.users`) have **no `profiles` row at all**, so this endpoint 404'd for them every time, meaning `_userRole` stayed `'admin'` forever and the tenant lived in the admin shell for their entire session — not just a brief flash. This is what was actually behind the very first bug reported this session ("tenant@boginenigroup.com has access to change the status" on the admin services screen) and, weeks later, why a freshly-created tenant login (Karthik V) still landed in the admin shell and had no notification bell (the tenant shell's top bar never had one — see `tenant-portal.md`). Fixed by making `/auth/profile` check the `tenants` table (via `auth_user_id`) first, since that's the actual ground truth for "is this user a tenant" — `profiles` is not reliably populated. The 2-tier frontend logic and the theoretical race documented below are unchanged and still apply, but are no longer masked by a 404 that always resolved to the wrong role.

## Known Issues
- Role resolution has a race/consistency gap: the JWT-decoded role is used first and immediately reflected in `isTenant`/redirects; the backend fallback (`fetchRole`) runs asynchronously afterward and can flip the role (and trigger another redirect) shortly after the first navigation, causing a visible flash to the wrong shell if the JWT hook is disabled and the user is actually a tenant (since only the 'admin' default triggers backend re-check, not other unexpected values). This is now a brief flash at worst (assuming a `tenants` row exists), not a permanent misroute — see Recent changes above.
